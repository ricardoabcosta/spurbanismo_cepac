...
**Status**: ✅ **Sessão 1 concluída** · ✅ **Sessão 2 concluída** · ✅ **Sessão 3 concluída** (2026-05-06)
**Origem**: planilha `docs/novos/OUCFL_ESTOQUE_abr_rv02.xlsx` + `20250805_workshop_OUCFL_part1.pdf`
**Data**: 2026-05-05
**Última atualização**: 2026-05-05 (reflete execução até o momento)
...
## 5. Sequência de migrations ✅ (Todas criadas)

| Migration | Conteúdo | Status |
|---|---|---|
| `025_lei_ouc_estoque.sql` | Cria `lei_ouc` e `setor_estoque_lei` | ✅ Criada e aplicada |
| `026_seed_lei_ouc.sql` | Popula `lei_ouc` para OUCAE (1 lei vigente), OUCAB (1 lei vigente), OUCFL (3 leis: 11.732 anterior, 13.769 anterior, 18.175 vigente) | ✅ Criada e aplicada |
| `027_seed_setor_estoque_lei.sql` | Popula `setor_estoque_lei` espelhando os atuais `setor.*` para cada `lei_ouc.vigente` + insere registros adicionais para OUCFL Lei 13.769 com validação de contagem | ✅ Criada e aplicada |
| `028_proposta_lei_beneficios.sql` | Adiciona campos novos em `proposta` (`lei_vigente`, 4 campos de benefícios) | ✅ Criada e aplicada |
| `029_ampliar_interessado.sql` | Amplia `proposta.interessado` de `VARCHAR(300)` para `TEXT` (necessário para interessados compostos com múltiplas empresas/pessoas na OUCFL) | ✅ Criada e aplicada |

`MIGRATION_ORDER` em `tests/integration/conftest.py` atualizado para incluir 025–029. ✅

> **Nota**: todas as migrations foram executadas no banco de produção (Azure) em 2026-05-06.

## 6. Mudanças no código backend

### 6.1 Models SQLAlchemy ✅ (Todos implementados)
- ✅ **Novo** `src/core/models/lei_ouc.py` (`LeiOuc`) — tabela `lei_ouc` com relacionamentos
- ✅ **Novo** `src/core/models/setor_estoque_lei.py` (`SetorEstoqueLei`) — tabela `setor_estoque_lei` com FK `setor_id` (UUID)
- ✅ `Proposta`: adicionados 5 campos — `lei_vigente`, `aca_r_real_m2`, `aca_r_beneficios_m2`, `aca_nr_real_m2`, `aca_nr_beneficios_m2`
- ✅ `Setor`: adicionado relationship `estoques_por_lei: list[SetorEstoqueLei]`
- ✅ `OperacaoUrbana`: adicionado relationship `leis: list[LeiOuc]`
- ✅ `__init__.py`: `LeiOuc` e `SetorEstoqueLei` registrados no metadata e `__all__`

### 6.2 Repositórios ✅ (Implementados)
- ✅ `saldo_repository.calcular_saldo()`: suporte a lei_vigente opcional via `get_limites_setor()`
- ✅ `saldo_repository.get_limites_ouc()`: lê `lei_ouc.estoque_geral_m2` para capacidade_global_m2
- ✅ `dashboard_repository`: sem mudanças — consumo histórico modelado via TituloCepac batch no script de carga

### 6.3 Engine de regras ✅ (Implementado)
- ✅ Validators usam `LimitesSetorDTO` e `LimitesOucDTO` lidos dos repositórios

### 6.4 Endpoints admin ✅ (Implementados)
- ✅ `GET /admin/leis` — listar leis, opcionalmente filtradas por `operacao_urbana_id`
- ✅ `POST /admin/leis` — criar lei (valida OUC e unicidade)
- ✅ `PUT /admin/leis/{id}` — atualizar lei (campos opcionais)
- ✅ `GET /admin/estoques-lei` — listar estoques, opcionalmente filtrados por `lei_ouc_id` ou `setor_id`
- ✅ `POST /admin/estoques-lei` — criar registro de estoque (valida unicidade setor × lei)
- ✅ `POST /portal/propostas` — aceita `lei_vigente` e benefícios opcionais

### 6.5 Frontend ✅ (Implementado — Sessão 2)
- ✅ `OUCAdminPage`: botão "Leis" por OUC abre painel lateral com histórico cronológico + CRUD (DIRETOR)
- ✅ `SetoresPorOUCPage`: seção "Estoques por Lei" com seletor de lei, tabela de estoques + criação (DIRETOR)
- ✅ Tipos TypeScript: `LeiOucOut`, `LeiOucIn`, `LeiOucUpdate`, `SetorEstoqueLeiOut`, `SetorEstoqueLeiIn` em `types/api.ts`
- ✅ API functions: `listarLeis`, `criarLei`, `atualizarLei`, `listarEstoquesLei`, `criarEstoqueLei` em `api/admin.ts`
- ⏳ Formulário de nova proposta no portal: dropdown `lei_vigente` — roadmap

## 7. Script de carga OUCFL ✅ (Implementado)

`scripts/carga_oucfl.py` — idempotente, transacional, com flag `--dry-run`. **Implementado.**
...
3. Atualiza contadores em `setor` e `operacao_urbana`
4. Atualiza `lei_ouc` da OUCFL com `consumo_historico_*`
5. Loga relatório final
...
|---|---|---|
| **1 — Modelagem + migrations** | Migrations 025–028, models, endpoints admin, schemas Pydantic, `MIGRATION_ORDER` atualizado | ✅ **Concluída** (05/05) |
| **2 — Script de carga + UI admin** | `scripts/carga_oucfl.py`, telas admin atualizadas para `lei_ouc` e `setor_estoque_lei`, repositórios e engine atualizados, deploy frontend | ✅ **Concluída** (05/05) |
| **3 — Carga real e validação** | Migration 029 (interessado TEXT), carga no Azure: 167 propostas, 262 certidões, 36 TituloCepac (8 hist. Lei 13.769 + 28 ind. Lei 18.175), validação pós-carga executada | ✅ **Concluída** (06/05) |
